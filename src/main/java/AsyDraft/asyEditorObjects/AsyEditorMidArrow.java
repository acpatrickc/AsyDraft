...
public class AsyEditorMidArrow extends AsyEditorSegment {
	 ...
	 */
	public AsyEditorMidArrow(double[] start, double[] end, AsyPen p) {
		super(start, end, p);
	}
	/*
	...
	public AsyObject getAsyObject(int gridwidth, int gridheight) {
		return new AsyMidArrow(MathUtils.toCartesian(startx, gridwidth), MathUtils.toCartesian(starty, gridheight), MathUtils.toCartesian(endx, gridwidth), MathUtils.toCartesian(endy, gridheight), pen);
	}
	/*
